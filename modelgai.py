...
class MobileViTBlock(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        fm = self.fusion(torch.cat((res, re, fm), dim=1))

        # fm = self.fusion(torch.cat((re, fm), dim=1))
        return fm
...
class MobileViT(nn.Module):
    ...
    """
    def __init__(self, model_cfg: Dict, num_classes: int = 1000) -> object:#model_cfg: Dict,
        super().__init__()

        image_channels = 3#原来是3
        out_channels = 16
